...
export default function Bookings() {
  ...
  const userOrdersFuture = useMemo(() =>
    userOrders
      .filter(d => d.event.status === 'live' && d.orderStatus !== 'canceled' && d.orderStatus !== null && d.state !== 0)
  ...
  const getUserOrders = async () => {
    const { data, error }: any = await supabase // any is required as currently the typing doesn't recognize eventminimal correctly
      .from('orders')
      .select(select.order)
      .neq('state', -1)  // Exclude stale orders
      .neq('state', 0);  // Exclude checkout orders (canceled orders are included in past)
  ...
  const renderBooking = useCallback((d: ListRenderItemInfo<Order>, isPastTab: boolean = false) => {
    ...
    // For past tab, show Completed or Refunded
    const isRefunded = d.item.orderStatus === 'canceled' || d.item.orderStatus === 'refunded' || d.item.orderStatus === 'declined';
    const isPastEvent = d.item.event.status === 'past' || d.item.event.status === 'cancelled';
    ...
    return (
      <Pressable style={styles.booking} onPress={() => navigation.navigate('BookingDetail', { orderID: d.item.id })}>
        ...
        <View style={styles.details}>
          ...
          {isPastTab && isPastEvent && (
            <>
              {isRefunded ? (
                <View style={styles.statusPill}>
                  <MaterialCommunityIcons name={'cash-refund'} size={16} color={theme.color.textBad} />
                  <AppText size={'small'} style={{ color: theme.color.textBad }}>Refunded</AppText>
                </View>
              ) : (
                <View style={styles.statusPill}>
                  <MaterialCommunityIcons name={'check-circle'} size={16} color={theme.color.primary} />
                  <AppText size={'small'} style={{ color: theme.color.primary }}>Completed</AppText>
                </View>
              )}
            </>
          )}
          {!isPastTab && d.item.orderStatus === 'confirmed' && (
            <View style={styles.statusPill}>
          ...
          {!isPastTab && d.item.orderStatus === 'pending' && (
            <View style={styles.statusPill}>
    ...
    >
      ...
      <Tabs.Tab name="Future">
        <Tabs.FlatList
          ...
          renderItem={(item) => renderBooking(item, false)}
        />
      </Tabs.Tab>
      ...
      <Tabs.Tab name="Past">
        <Tabs.FlatList
          ...
          contentContainerStyle={styles.tabContainer}
          data={userOrdersPast}
          renderItem={(item) => renderBooking(item, true)}
        />
      </Tabs.Tab>
    </Tabs.Container>
  </View>
