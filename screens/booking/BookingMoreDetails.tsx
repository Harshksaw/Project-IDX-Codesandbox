...
export default function BookingMoreDetails(props: { route: any }) {
  ...
  // cancel pending order (uses decline action)
  const cancelPendingOrder = useCallback(async () => {
    ...
    if(refreshSessionError) {
      ...
      Toast.show({
        type: 'error',
        text1: 'Failed to prepare order cancellation.'
      });
      return;
    }

    let cancelOrderRes: Response;
    try {
      cancelOrderRes = await fetch(endpoints.checkout.confirmOrder, {
        method: 'POST',
        headers: {
    ...
    if(cancelOrderRes.status !== 200) {
      const message = await cancelOrderRes.text();
    ...
    // triggers a refresh for bookings to re-fetch updated order data
  ...
  return <AppSafeAreaView>
    ...
    <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollViewContent}>
      ...
      <View style={styles.cancelSection}>
        {(userOrder.state === -3 || userOrder.state === -2) ? (
          <View style={styles.refundedButton}>
            <MaterialCommunityIcons name={'cash-refund'} size={20} color={theme.color.textBad} />
            <AppText size={'small'} font={'heavy'} style={{ color: theme.color.textBad }}>Refunded</AppText>
          </View>
        ) : (
          (userOrder.orderStatus === 'pending' || userOrder.orderStatus === 'confirmed') && !userOrder.checkedIn && (
            <Pressable 
              style={[
                styles.cancelButton,
                (userOrder.orderStatus === 'confirmed' && firstListing.refundTimeLimit && new Date(firstListing.refundTimeLimit) < new Date()) ? styles.cancelButtonDisabled : undefined
              ]} 
              onPress={handleRequestRefund}
              disabled={userOrder.orderStatus === 'confirmed' && !!(firstListing.refundTimeLimit && new Date(firstListing.refundTimeLimit) < new Date())}
            >
    ...
    <ConfirmationSlideOver show={showRefundSlideOver}
                           ...
                           title={userOrder.orderStatus === 'pending' ? 'Confirm cancellation' : 'Confirm refund'}
                           body={userOrder.orderStatus === 'pending' 
                             ? `This order will be canceled and the payment will not be captured. All listings bought in this order will be cancelled.`
                             : `You will be refunded ${formatCents(cost.dueNow - cost.totalFees)}, not including the order fee. All listings bought in this order will be cancelled. You may only request a refund before ${formatInTimeZone(firstListing.refundTimeLimit, timezone, 'MMM d, h:mm aaa')}.`}
                           rightButtonHandle={userOrder.orderStatus === 'pending' ? cancelPendingOrder : refundOrder}
                           leftButtonText={'Cancel'}
                           rightButtonText={userOrder.orderStatus === 'pending' ? 'Cancel order' : 'Request refund'}
    />
  </AppSafeAreaView>
}
