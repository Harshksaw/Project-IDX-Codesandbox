...
export default function PurchaseConfirmed(props: { route: any }) {
  ...
  const [orderStatus, setOrderStatus] = useState<'confirmed' | 'pending' | 'declined' | 'canceled' | 'checkout' | 'stale' | 'refunded' | null>(null);
  ...
  }, [finalized, isPending, orderID, navigation]);
  ...
  }, [finalized, isPending, navigation]);
  ...
  return <AppSafeAreaView style={styles.container}>
    ...
    <View style={styles.middle}>
      {
        isPending
          ? <>
            ...
            />
            <AppText font={'black'} size={'large'}>Order is pending</AppText>
            <AppText size={'small'}>Once accepted order will be confirmed</AppText>
          </>
          : finalized
            ? <>
              <LottieView
    ...
    <View style={styles.bottom}>
      <GradientButton style={styles.viewDetails} onPress={handleViewBookingDetails} active={finalized || isPending}>
      ...
      <Pressable onPress={handleClose} style={styles.close}>
        <AppText font={'heavy'} color={finalized || isPending ? 'textPayment' : 'secondary'}>Close</AppText>
      </Pressable>
    </View>
  </AppSafeAreaView>
}
